...
def solicitar_exames(request):
    ...
    if request.method == "GET":
        ...
        exames = TiposExames.objects.filter(id__in=exames_id)
    
        #TODO: calcular preço disponível
        preco_total = 0
...
def fechar_pedido(request):
    ...
    messages.add_message(request, constants.SUCCESS, 'Pedido de Exames recebido no sistema!')
...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        messages.add_message(request, constants.ERROR, 'Não Autorizado! Pedido não pertence ao Usuário!')
        return redirect('/exames/gerenciar_pedidos/')
...
def gerar_acesso_medico(request):
    if request.method == "GET":
        ...
        messages.add_message(request, constants.SUCCESS, 'Link de Acesso Gerado!')
...
def acesso_medico(request, token):
    ...
    if acesso_medico.status == 'Expirado':
        messages.add_message(request, constants.WARNING, 'Link de Acesso EXPIRADO!')

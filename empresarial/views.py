...
def alterar_dados_exame(request, exame_id):
    ...
    if requer_senha and (not exame.senha):
        messages.add_message(request, constants.ERROR, 'PDF sem Senha criada! Não pode exigir senha!')
    ...
    exame.status = status
    exame.save()
    messages.add_message(request, constants.SUCCESS, 'Dados do exame alterados!')
    return redirect(f'/empresarial/exame_cliente/{exame_id}')
